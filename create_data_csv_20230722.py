...
def create_validate_df(all_df):
    validated_df = all_df[all_df["object_annotation_status"]=="validated"].copy()
    return validated_df
...
def create_label_overview(validated_df):
    ...
    overview_labels.to_csv('output/annotation_overview.csv',sep=";")
...
def create_data_split(validated_df_zero):
    df_train, df_val_test = train_test_split(validated_df_zero, test_size=0.3, stratify=validated_df_zero['label'])
...
if __name__ == '__main__':
    ...
    all_df.to_csv('output/update_allcruises_df_5with_zoomie_20230722.csv', sep=";")
    validated_df = create_validate_df(all_df)
    overview_labels, overview_labels_100 = create_label_overview(validated_df)
    dict_babsi = create_agg_labels(path ="data/combine_classes_for_training_20230722.csv")
    ...
    df_phase.to_csv('output/update_allcruises_df_validated_5with_zoomie_20230722.csv',sep=";")
    list_of_folders = list(np.unique(validated_df_zero['label']))
    print(len(list_of_folders))
    print(list_of_folders)
    #tvt = ['train', 'val', 'test']
    #create_folders(tvt, list_of_folders, run_id="data_set_124")
    #build_tvt(df_phase, run_id="data_set_124")
