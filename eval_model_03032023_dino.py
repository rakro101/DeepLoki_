...
if __name__ == '__main__':
    ...
    model = DtlModel(input_shape=(3,300,300), num_classes=num_classes, arch="resnet_dino", label_encoder=le)
    ...
    model.eval()
    run_prefix = c_path.split("/")[1]+'new_dino'
    print(run_prefix)
    ...
    if dl_model:
        ...
        clsf_report.to_csv(f"paper/tables/SSV_Dino_{run_prefix}_resnet18_class_report.csv", sep=";")
        print(clsf_report)
        ...
        plt.tight_layout()
        plt.savefig(f'paper/figures/SSV_Dino_{run_prefix}_resnet_18__confusion_matrix.jpg')
        plt.show()
